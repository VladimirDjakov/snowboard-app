...
class TestGetFile:
    ...
    def test_get_file_success_mp4(self, client, mock_container):
        ...
        response = client.get(f"/api/v1/files/{object_key}")

        assert response.status_code == 200
        assert response.content == file_data
        assert response.headers["content-type"] == "video/mp4"
        assert f'filename="{object_key.split("/")[-1]}"' in response.headers["content-disposition"]

        mock_container.storage.exists.assert_called_once_with(object_key)
        mock_container.storage.read_file.assert_called_once_with(object_key)

    def test_get_file_success_jsonl(self, client, mock_container):
        """Test successful file download for JSONL file."""
        video_id = uuid4()
        object_key = f"proc/{video_id}/keypoints_v1.jsonl"
        file_data = b'{"frame": 1}\n{"frame": 2}\n'
    ...
    def test_get_file_success_json(self, client, mock_container):
        ...
        video_id = uuid4()
        object_key = f"proc/{video_id}/features_v1.json"
    ...
    def test_get_file_url_encoded_key(self, client, mock_container):
        ...
        video_id = uuid4()
        object_key = f"raw/{video_id}/original video.mp4"
