...
@pytest.fixture
def sample_video(test_session: Session) -> Video:
...
class TestVideo:
    ...
    def test_video_unique_share_token(self, test_session: Session, sample_video: Video):
    ...
    def test_video_relationships(self, test_session: Session, sample_video: Video):
        ...
        artifact = Artifact(
            ...
            version="v1",
            object_key="raw/test/original.mp4",
        )
...
class TestJobStage:
    ...
    def test_create_job_stage(self, test_session: Session, sample_video: Video):
    ...
    def test_job_stage_unique_constraint(self, test_session: Session, sample_video: Video):
    ...
    def test_job_stage_status_transitions(self, test_session: Session, sample_video: Video):
    ...
    def test_job_stage_error_message(self, test_session: Session, sample_video: Video):
    ...
    def test_job_stage_cascade_delete(self, test_session: Session):
        ...
        deleted_stage = test_session.get(JobStage, stage_id)
...
class TestArtifact:
    ...
    def test_create_artifact(self, test_session: Session, sample_video: Video):
        """Test creating an artifact."""
        artifact = Artifact(
            video_id=sample_video.id,
            kind=ArtifactKind.NORMALIZED,
            version="v1",
            object_key="raw/test/normalized.mp4",
        )
        test_session.add(artifact)
        test_session.commit()
        ...
        assert artifact.object_key == "raw/test/normalized.mp4"
        assert artifact.created_at is not None

    def test_artifact_unique_constraint(self, test_session: Session, sample_video: Video):
        """Test that (video_id, kind, version) must be unique."""
        artifact1 = Artifact(
            video_id=sample_video.id,
            kind=ArtifactKind.KEYPOINTS,
            version="v1",
            object_key="raw/test/keypoints_v1.jsonl",
        )
        ...
        artifact2 = Artifact(
            ...
            object_key="raw/test/keypoints_v1_duplicate.jsonl",
        )
    ...
    def test_artifact_different_versions(self, test_session: Session, sample_video: Video):
        """Test that same kind with different versions is allowed."""
        artifact_v1 = Artifact(
            video_id=sample_video.id,
            kind=ArtifactKind.FEATURES,
            version="v1",
            object_key="raw/test/features_v1.json",
        )
        artifact_v2 = Artifact(
            video_id=sample_video.id,
            kind=ArtifactKind.FEATURES,
            version="v2",  # Different version
            object_key="raw/test/features_v2.json",
        )
    ...
    def test_artifact_all_kinds(self, test_session: Session, sample_video: Video):
        ...
        for kind in kinds:
            artifact = Artifact(
                ...
                kind=kind,
                version="v1",
                object_key=f"raw/test/{kind.value}_v1",
            )
            test_session.add(artifact)
    ...
    def test_artifact_cascade_delete(self, test_session: Session):
        ...
        deleted_artifact = test_session.get(Artifact, artifact_id)
    ...
    def test_video_with_multiple_artifacts(self, test_session: Session):
        ...
        artifacts = [
            ...
            Artifact(
                ...
                version="v1",
                object_key="raw/test/feedback_v1.json",
            ),
        ]
