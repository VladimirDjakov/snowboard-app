...
class TestLocalStorage:
    ...
    def test_generate_upload_url_creates_directory(self, tmp_path: Path):
        ...
        storage_path = "raw/video123/original.mp4"
        ...
        file_path = backend._get_file_path(storage_path)
        assert file_path.parent.exists()

        # Check UploadInfo
        assert isinstance(upload_info, PresignedUploadUrl)
        assert upload_info.method == "PUT"
        assert upload_info.storage_path == storage_path
    ...
    def test_generate_download_url(self, tmp_path: Path):
        ...
        backend = LocalStorage(tmp_path.joinpath("storage"))
        storage_path = "proc/video123/normalized.mp4"

        url = backend.generate_download_url(storage_path, expires_in=1800)

        assert "/api/v1/files/" in url
        assert storage_path in url or "video123" in url
    ...
    def test_exists_file_exists(self, tmp_path: Path):
        ...
        assert backend.exists(storage_path) is True
    ...
    def test_exists_file_not_exists(self, tmp_path: Path):
        ...
        assert backend.exists(storage_path) is False
...
class TestS3Storage:
    ...
    def mock_boto3_client(self):
        ...
        with patch("backend.app.infrastructure.storage.s3.boto3") as mock_boto3:
    ...
    def test_init_raises_import_error_without_boto3(self):
        ...
        with patch("backend.app.infrastructure.storage.s3.boto3", None):
...
class TestStorageFactory:
    ...
    def test_create_s3_storage(self):
        ...
        with patch("backend.app.infrastructure.storage.s3.boto3"):
            backend = create_storage_backend(settings)
