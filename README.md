...
```
📮 Queue ──Задача normalize──> 💻 Normalize Worker
                                      │
...
| Сущность | Описание |
|----------|----------|
| `pipeline_version` | Версия конвейера (normalize → pose → …) |
...
- **`cpu/normalize/`** — нормализация видео с помощью ffmpeg
